...
impl From<&Sale> for SaleResponse {
    fn from(s: &Sale) -> Self {
        Self {
            ...
            shift_id: s.shift_id().map(|s| s.into_uuid()),
...
impl From<Sale> for SaleDetailResponse {
    fn from(s: Sale) -> Self {
        ...
        Self {
            ...
            currency: s.currency().as_str().to_string(),
            invoice_number: s.invoice_number(),
            cai_number: s.cai_number().map(String::from),
            invoice_date: s.invoice_date(),
            created_by_id: s.created_by_id().into_uuid(),
            confirmed_at: s.confirmed_at(),
            paid_at: s.paid_at(),
            cancelled_at: s.cancelled_at(),
            cancelled_by_id: s.cancelled_by_id().map(|u| u.into_uuid()),
            cancellation_reason: s.cancellation_reason().map(String::from),
