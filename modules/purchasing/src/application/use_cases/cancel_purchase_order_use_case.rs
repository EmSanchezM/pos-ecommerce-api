// CancelPurchaseOrderUseCase - cancels a purchase order
...
pub struct CancelPurchaseOrderUseCase<P>
where
...
impl<P> CancelPurchaseOrderUseCase<P>
where
    P: PurchaseOrderRepository,
{
    /// Creates a new instance of CancelPurchaseOrderUseCase
    pub fn new(order_repo: Arc<P>) -> Self {
        Self { order_repo }
    }

    /// Executes the use case to cancel a purchase order
    ///
    /// # Arguments
    /// * `order_id` - The ID of the purchase order to cancel
    /// * `command` - The cancel command with reason
    /// * `actor_id` - ID of the user cancelling the order
    ///
    /// # Returns
    ...
    pub async fn execute(
        ...
        // Cancel order
        order.cancel(actor_id, command.reason)?;
