// ClosePurchaseOrderUseCase - closes a fully received purchase order
...
/// Use case for closing a fully received purchase order
pub struct ClosePurchaseOrderUseCase<P>
where
...
impl<P> ClosePurchaseOrderUseCase<P>
where
    P: PurchaseOrderRepository,
{
    /// Creates a new instance of ClosePurchaseOrderUseCase
    pub fn new(order_repo: Arc<P>) -> Self {
        Self { order_repo }
    }

    /// Executes the use case to close a purchase order
    ///
    /// # Arguments
    /// * `order_id` - The ID of the purchase order to close
    ///
    /// # Returns
